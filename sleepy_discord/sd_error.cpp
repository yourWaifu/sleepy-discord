...
namespace SleepyDiscord {
	void DiscordClient::setError(int errorCode) {
		...
		//Disconnections
		case 4000: onError(code, "unknown error - We're not sure what went wrong. Try reconnecting?"                                                                  ); break;
		case 4001: onError(code, "unknown opcode - You sent an invalid Gateway OP Code. Don't do that!"                                                               ); break;
		case 4002: onError(code, "decode error - You sent an invalid payload to us. Don't do that!"                                                                   ); break;
		case 4003: onError(code, "not authenticated - You sent us a payload prior to identifying."                                                                    ); break;
		case 4004: onError(code, "authentication failed - The account token sent with your identify payload is incorrect."                                            ); break;
		case 4005: onError(code, "already authenticated - You sent more than one identify payload. Don't do that!"                                                    ); break;
		case 4007: onError(code, "invalid seq - The sequence sent when resuming the session was invalid. Reconnect and start a new session."                          ); break;
		case 4008: onError(code, "rate limited - Woah nelly!You're sending payloads to us too quickly. Slow it down!"                                                 ); break;
		case 4009: onError(code, "session timeout - Your session timed out.Reconnect and start a new one."                                                            ); break;
		case 4010: onError(code, "invalid shard - You sent us an invalid shard when identifying."                                                                     ); break;
		case 4011: onError(code, "sharding required - The session would have handled too many guilds - you are required to shard your connection in order to connect."); break;
		}																															   
	}
}